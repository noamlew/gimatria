...
plus.addEventListener("click", () => {
    ...
    dt1.innerHTML =
        ...
        " + ";
    allDt.push([dt1, milim[milim.length - 1]["value"]]);
    allDt.forEach((oneDt) => {
        if (oneDt[1] === milim[milim.length - 1]["value"]) {
            oneDt[0].style.color = "blacl"
        } else {
            oneDt[0].style.color = "grey";
        }
    });
...
document.addEventListener("keydown", (event) => {
    if (event.keyCode === 13) {
        ...
        allDt.forEach((oneDt) => {
            if (oneDt[1] === milim[milim.length - 1]["value"]) {
                // console.log(milim[milim.length - 1]["name"])
                oneDt[0].style.color = "black"
            } else {
                oneDt[0].style.color = "grey";
            }
        });
...
shave.addEventListener("click", () => {
    ...
    dt.innerHTML =
        ...
        "  -  " +
        String(milim[milim.length - 1]["value"]);
    allDt.push([dt, milim[milim.length - 1]["value"]]);
    allDt.forEach((oneDt) => {
        oneDt[0].style.color = "black";
    });
...
bitul.addEventListener("click", () => {
    ...
    milim.forEach((mila) => {
        ...
        allDt.push([dt1, mila["value"]]);
        if (mila["value"] === milim[milim.length - 1]["value"]) {
            allDt[allDt.length - 1][0].style.color = "black";
        } else {
            allDt[allDt.length - 1][0].style.color = "grey";
        }
        output.appendChild(dt1);
        input.value = "";
    });
...
function target() {
    ...
    targetP.innerHTML = (sumOfGimatria(targetInput.value) - sum) * -1;
}
